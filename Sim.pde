...
class Sim 
{
  ...
  */
  void display() 
  {
    for ( int i = 0; i < columns; i++) 
    {
      for ( int j = 0; j < rows; j++) 
      {
        if ((board[i][j] == 2)) fill(255, 0, 50); //Pred
        else if ((board[i][j] == 1)) fill(102, 250, 155); //Prey
        else if ((board[i][j] == 0)) fill(0); //Blank 
        stroke(0);
  ...
  */  
  int predPreyPicker()
  {
    int rtn;
    float r = random(0, 1);
    if (r < 0.3) rtn = 1; //green prey30%
    else if (r < 0.5) rtn = 2; //Rered20%
    else rtn = 0; //blank empty 50%
    return rtn;
  }
