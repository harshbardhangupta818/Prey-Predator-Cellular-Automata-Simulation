  ...
  */
  ...
  //User input handler
  {
    ...
    output = createWriter("results/second_new_results.m"); 
  }
  ...
  */
  void userInputHandler(){
    ...
    String op1s = JOptionPane.showInputDialog(null, "How many generations would you like the simulation to run for?", 500);
    if(op1s != null) op1=Integer.parseInt(op1s);
    String op2s = JOptionPane.showInputDialog(null, "What percentage of the board would you like filled initially?", 10);
    if(op2s != null) op2=Integer.parseInt(op2s);
    String op3s = JOptionPane.showInputDialog(null, "What would you like the predator catch rate to be? (0.0 - 1.0)", 0.33);
    if(op3s != null) op3=java.lang.Float.parseFloat(op3s);
    op4 = JOptionPane.showInputDialog(null, "What method of distribution would you like to use? Stochastic or deterministic? (s or d)", "s");
  }
